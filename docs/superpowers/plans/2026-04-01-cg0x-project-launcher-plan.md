# cg0x-project-launcher Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 创建 `cg0x-project-launcher` skill，合并 init-maker + dev-standards 的全部功能。

**Architecture:** 单文件 SKILL.md（约400-450行），按8阶段线性Pipeline执行，生成双系统启动脚本（`.cmd` + `.sh`），覆盖环境探测→脚手架→依赖→凭证→nginx→服务启动→ngrok→健康验证全流程。

**Tech Stack:** Batch（Windows）、Bash（macOS）、Python stdlib、nginx、ngrok
...
| File | Role |
|------|------|
| `skills/cg0x-project-launcher/SKILL.md` | 单一skill文件，包含所有规范和脚本模板 |
| `.claude-plugin/marketplace.json` | skill注册表，移除init-maker/dev-standards，添加project-launcher |
| `README.md` | skill列表展示，需同步更新 |

---

## Task 1: 创建目录

- [ ] 创建 `skills/cg0x-project-launcher/` 目录

```bash
mkdir -p skills/cg0x-project-launcher
```

---

## Task 2: 编写 SKILL.md（约420行）

**文件**: `skills/cg0x-project-launcher/SKILL.md`
...
if %WINGET_AVAILABLE%==1 (
    ...
    if /i "!ANS!"=="Y" winget install Python.Python.3.12 --accept-source-agreements --accept-package-agreements
)
if !ANS! neq Y (
...
check_python() {
  ...
  read -p "After installing, press ENTER to re-check..."; check_python
}
```
...
列出挂掉的链路 + 修复建议，不自动尝试修复（由用户决定是否触发 service-guardian）。
